...
function OneWeekStock({ symbol }) {
  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      try {
        const response = await fetch(`http://localhost:5000/stock_history_one_week/${symbol}`);
        if (!response.ok) {
  ...
  const renderChart = () => {
    const ctx = document.getElementById('one-week-chart').getContext('2d');
    ...
    const newChart = new Chart(ctx, {
      ...
      options: {
        plugins: {
          title: {
            display: true,
            text: `One Week Historical Data for ${symbol}`,
            font: {
              size: 16
            }
          },
  ...
  return (
    <div className="stock-card stock-card-chart">
      <canvas id="one-week-chart" style={{ width: '100%', height: '400px' }}></canvas>
    </div>
  );
}

export default OneWeekStock;
